...
inline constexpr auto properties(blob::tag<ExampleStruct>) {
    static_assert(std::is_empty_v<ExampleStruct> == false);
    static_assert(std::is_array_v<ExampleStruct> == false);
    static_assert(std::is_fundamental_v<ExampleStruct> == false);
